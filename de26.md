...
systemctl restart sshd
apt-get install admc libsss_sudo sudo nfs-clients chrony yandex-browser-stable -y
...
systemctl restart dnsmasq
apt-get update && apt-get install fdisk nfs-server chrony lamp-server -y
```
...
apt-get update && apt-get install task-samba-dc sudo-samba-schema chrony ansible docker-compose-v2 docker-engine -y
vim /etc/resolv.conf
